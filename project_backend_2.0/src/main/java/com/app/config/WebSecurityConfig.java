		...
		http.cors().and().csrf().disable().exceptionHandling().authenticationEntryPoint((request, response, ex) -> {
			response.sendError(HttpServletResponse.SC_UNAUTHORIZED, ex.getMessage());
				...
				.antMatchers("/order/active/{customerId}", "/order/completed/{customerId}",
						"/order/pending/{customerId}")/* .permitAll() */
				.hasRole("CUSTOMER")
				.antMatchers("/edit/customer", "/edit/customer/emailExist",
						"/edit/customer/contactExists")/* .permitAll() */
				.hasRole("CUSTOMER")
				.antMatchers("/order/place", "/order/cancel/{orderId}", "/order/setcompleted",
						"/order/finalize")/* .permitAll() */
				.hasRole("CUSTOMER")
...
//				.hasRole("CUSTOMER")
				...
				.antMatchers("/edit/vendor", "/edit/vendor/emailExist", "/edit/vendor/contactExists",
						"/order/completed")/* .permitAll() */
				.hasRole("VENDOR")
				.antMatchers("/order/pending", "/order/request", "/order/placebid",
						"/order/deletebid")/* .permitAll() */
				.hasRole("VENDOR")
...
//				.hasRole("VENDOR")
				...
				.antMatchers("/**", "/auth/**", "/swagger*/**", "/v*/api-docs/**").permitAll() //ganesh added
